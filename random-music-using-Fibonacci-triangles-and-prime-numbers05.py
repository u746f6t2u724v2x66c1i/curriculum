...
# 数列を使って音楽を再生する関数
def play_variation_music(fibonacci, triangle, primes, device_id, duration=1500):
    pygame.midi.init()
    ...
    try:
        ...
        while time.time() - start_time < duration:
            ...
            # 追加のバリエーション：数列の選択を変える
            if random.random() < 0.1:  # 10%の確率で数列を変更
                selected_sequence = random.choice([fibonacci_notes, triangle_notes, prime_notes])
...
if __name__ == "__main__":
    ...
    # 音楽を再生（3分間）
    play_variation_music(fibonacci, triangle, primes, output_device_id)
