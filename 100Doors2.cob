...
Begin.
    ...
    PERFORM VARYING StepSize FROM 1 BY 2 UNTIL StepSize > 100
      PERFORM VARYING Current-n FROM StepSize BY 2 UNTIL Current-n > 100
        SUBTRACT Doors (Current-n) FROM 1 GIVING Doors (Current-n)
      END-PERFORM
    END-PERFORM

    PERFORM VARYING Idx FROM 1 BY 1 UNTIL Idx > 100
        IF ClosedDoor (Idx) THEN
